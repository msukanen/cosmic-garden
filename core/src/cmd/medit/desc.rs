...
mod medit_desc_tests {
    ...
    async fn medit_desc() {
        ...
        let state = ctx!(xsup sup, state, MeditCommand, "goblin", s,c,w,|out:&str| out.contains("Huh?"));
        p.write().await.access = Access::Builder;
        let state = ctx!(xsup sup, state, MeditCommand, "goblin", s,c,w,|out:&str| out.contains("nvoked"));
        let state = ctx!(xsup sup, state, RenameCommand, "Morg-Gluglug",s,c,w,|out:&str| out.contains("renamed"));
