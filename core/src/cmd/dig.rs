...
use crate::{cmd::{Command, CommandCtx}, io_thread::WORLD_NEEDS_SAVING, player_or_bust, room::Room, string::Slugger, tell_user, tell_user_unk, util::{access::Accessor, direction::Direction}};
...
impl Command for DigCommand {
    async fn exec(&self, ctx: &mut CommandCtx<'_>) {
        ...
        let target_arc = if let Some(existing) = dest_room {
            ...
            match Room::new(&dest_id, &new_title).await {
                Ok(r) => {
                    ...
                    w.rooms.insert(dest_id.clone(), r.clone());
        ...
        if let Err(e) = origin_arc.write().await.link_exit(ctx.world.clone(), dir.clone(), &dest_id).await {
            ...
            // persist both rooms
            let _ = origin_arc.write().await.save().await;
            let _ = target_arc.write().await.save().await;
