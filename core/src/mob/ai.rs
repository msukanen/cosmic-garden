//! Mob "AI" (eye-I) stuff.

use serde::{Deserialize, Serialize};

use crate::{room::environ::{SpecialEnvironment, Terrain, WEATHER_RAIN}, single_tick_meaning, traits::TickMeaning};

#[derive(Debug, Clone, Copy, Deserialize, Serialize, PartialEq)]
pub enum AiState {
    Idle,
    Wandering,
}

impl Default for AiState {
    fn default() -> Self {
        Self::Idle
    }
}

#[derive(Debug, Clone, Copy, Deserialize, Serialize, PartialEq)]
pub enum AiMentalState {
    Happy,
    Neutral,
    Grumpy,
    Angry,
}

impl Default for AiMentalState {
    fn default() -> Self {
        Self::Neutral
    }
}

#[derive(Debug, Clone)]
pub enum AiAction {
    Emote { fmt: String },
    //Attack { xyz }
}

#[derive(Debug, Clone, Deserialize, Serialize)]
pub struct Ai {
    state: AiState,
    mental_state: AiMentalState,

    #[serde(skip, default = "ai_rng_default")] rng: u64,
    p_idle_to_wandering: f32,
    p_wandering_to_idle: f32,
}

fn ai_rng_default() -> u64 {
    rand::random::<u64>()
}

#[inline] fn ai_rng(seed: u64) -> u64 { seed.wrapping_mul(6364136223846793005).wrapping_add(1) }
#[inline] const fn ai_probability(base: u64) -> f32 { (base >> 32) as f32 / 4294967296.0 }

impl Default for Ai {
    fn default() -> Self {
        Self { 
            state: AiState::default(),
            mental_state: AiMentalState::default(),
            rng: ai_rng_default(),
            p_idle_to_wandering: 0.01,
            p_wandering_to_idle: 0.05,
        }
    }
}

impl Ai {
    pub fn tick(&mut self, e_title: &str, curr_tick: usize, room_env: SpecialEnvironment, room_terrain: Option<Terrain>) -> Option<Vec<TickMeaning> > {
        let mut maybe_state = None;
        let mut maybe_mental_state = None;
        let mut maybe_action = None;
        self.rng = ai_rng(self.rng);

        match self.state {
            AiState::Idle => {
                if ai_probability(self.rng) <= 0.01 {
                    self.state = AiState::Wandering;
                    maybe_state = self.state.into();
                }
            }

            AiState::Wandering => {
                // move?
                if ai_probability(self.rng) <= 0.05 {
                    self.state = AiState::Idle;
                    maybe_state = self.state.into();
                }
            }
        }

        if room_env & WEATHER_RAIN != 0 && self.mental_state == AiMentalState::Grumpy {
            // let's not spam too much…
            self.rng = ai_rng(self.rng);
            if ai_probability(self.rng) <= 0.01 {
                maybe_action = AiAction::Emote { fmt: format!("[{e_title}] glares at the clouds for a moment.") }.into()
            }
        }
        
        single_tick_meaning!(TickMeaning::AiStateChange { maybe_state, maybe_mental_state, maybe_action })
    }
}
